...
async def update_balance_handler(WALLET_UUID: uuid.UUID, operation: OperationSchema):
    return await update_wallet_balance(operation, WALLET_UUID)


@v1_router.get("/wallets/{WALLET_UUID}", response_model=WalletSchema)
async def get_wallet_handler(WALLET_UUID: uuid.UUID):
    return await get_wallet_by_id(WALLET_UUID)


@v1_router.post("/wallets/create/{WALLET_UUID}", status_code=status.HTTP_201_CREATED)
